			...
			.executes(
				context -> {
					...
					StoreProduct product = StoreProduct.builder()
						...
						.id("example_product")
						.price(1)
						.quantity(StoreProductQuantity.builder().min(1).value(1).max(1).build())
						.build();
					StoreProduct[] storeProducts = new StoreProduct[] {product};
					StoreRequest storeRequest = StoreRequest.builder().storeProducts(storeProducts).mcaccount(player.getUniqueId()).build();
