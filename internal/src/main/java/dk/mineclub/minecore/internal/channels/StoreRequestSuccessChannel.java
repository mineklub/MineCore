...
public class StoreRequestSuccessChannel extends JedisPubSub {
	...
	public void handleMessage(String message) {
		...
		parser.parse(messageObject.get("request").getAsJsonObject().getAsJsonObject("request").toString())
			.ifPresentOrElse(
				parsed -> {
					Player player = plugin.getServer().getPlayer(UUID.fromString(parsed.data().mcaccount().uuid())).orElse(null);
					if (player == null) {
						return;
					}

					if (player.getProtocolVersion().compareTo(ProtocolVersion.MINECRAFT_1_21_6) < 0) {
